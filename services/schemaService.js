...
export function normalizeOrder(doc) {
  ...
  const clientId = String(doc?.clientId ?? doc?.clienteId ?? "").trim();
  const clientNameRaw = String(doc?.clientName ?? doc?.clienteNome ?? doc?.cliente ?? doc?.nomeCliente ?? doc?.customerName ?? "").trim();
  const clientName = upperName(clientNameRaw);
  const residual = num(doc?.residual ?? doc?.residuo, 0);
  const status = String(doc?.paymentStatus ?? doc?.status ?? doc?.stato ?? "").trim().toLowerCase();
  const rows = Array.isArray(doc?.rows) ? doc.rows : (Array.isArray(doc?.items) ? doc.items : []);
  return {
    id,
    createdAtISO,
    total,
    ...
    rows,
    items: rows,
    note: String(doc?.note ?? doc?.descrizione ?? '').trim(),
  };
}
